...
// 				<h2 className='text-xl font-semibold text-black'>Employeers</h2>
...
const UsersTable = ({ subscriptions = [] }) => {
	const [searchTerm, setSearchTerm] = useState("");
	const [filteredUsers, setFilteredUsers] = useState(subscriptions);

	useEffect(() => {
		setFilteredUsers(subscriptions || []);
	}, [subscriptions]);
	...
	return (
		<motion.div className='shadow-lg rounded-xl p-6 border border-gray-700'
			initial={{ opacity: 0, y: 20 }}
			animate={{ opacity: 1, y: 0 }}
			transition={{ delay: 0.2 }}
		>
			{/* Search Bar */}
			<div className='flex justify-between items-center mb-6'>
				<h2 className='text-xl font-semibold text-black'>Employers</h2>
				<div className='relative'>
					<input
						type='text'
						placeholder='Search users...'
					...
					<table className='min-w-full divide-y divide-gray-700'>
						<thead>
							<tr>
								...
								<th className='px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider'>Tokens Available</th>
								<th className='px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider'>Tokens consumed</th>
								<th className='px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider'>Pending tokens</th>
							</tr>
						</thead>
								...
								>
									<td className='px-6 py-4 whitespace-nowrap'>
										<div className='flex items-center'>
											...
											<div className='ml-4'>
												<div className='text-sm font-medium text-black'>{user?.fullName || "N/A"}</div>
											</div>
										</div>
									</td>
									<td className='px-6 py-4 whitespace-nowrap text-sm text-black'>{user?.email || "N/A"}</td>
									<td className='px-6 py-4 whitespace-nowrap'>
										<span className='px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-800 text-blue-100'>
											{user?.role || "N/A"}
										</span>
